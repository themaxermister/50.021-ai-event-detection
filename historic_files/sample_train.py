...
def save_progress(model, train_losses, train_nce, train_recon, train_losses_dicts_mean,
    ...
    model_path = '../saved_models/SF/mapsed.torch'
    ...
    df_result.to_csv('../saved_models/SF/mapsed.csv')


device = torch.device('cuda')

seq_train, seq_valid, _ = load_seq_data('SF')
mean = seq_train.mean(axis=(0,1,3,4))
max = np.array([seq_train[:, :, i].max() for i in range(4)])
normalizer = max
seq_train = normalize_data(seq_train, normalizer)
seq_valid = normalize_data(seq_valid, normalizer)

dataloader_train = DataLoader(seq_train, 32, True, drop_last=True)

dataloader_vallid = DataLoader(seq_valid, 32, True, drop_last=True)
epochs = 201
...
model = MAPSED(vae, latent_shape=(2, 5, 5), m=m, n=n, lambda_contrast=5, contrast='L2',lambda_MAE=1).to(device)
...
pytorch_total_params = sum(p.numel() for p in model
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for _, seq in enumerate(dataloader_vallid):
            seq = seq.cuda().float()
            x = seq[:, :m]
            gt = seq[:, m:]
